...
六个 Gate 全部 `PASS` 才完成本地 Release Readiness。审计器不会自动创建 Tag；Founder 明确授权后先创建 annotated tag，再重跑 M6 验证其目标 Commit、版本与 Approval Reference 的绑定。Canonical Promotion、Push、外部部署与发布后验证仍是独立授权动作。
